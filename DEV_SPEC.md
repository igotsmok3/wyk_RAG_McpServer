...
- **自研 Pipeline 框架（设计灵感参考 LlamaIndex 分层思想，但不依赖 LlamaIndex 库）：**
	...
	- 与主流 embedding provider 有良好适配，架构中统一使用 Milvus 作为向量存储。
...
- **Upsert & Storage (索引存储)**
	- **存储后端**：统一使用向量数据库（如 Milvus/Qdrant）作为存储引擎，同时持久化存储 Dense Vector、Sparse Vector 以及 Transform 阶段生成的富 Metadata。
...
- **文档生命周期管理 (Document Lifecycle Management)**
	...
	- **DocumentManager（文档管理器）**：独立于 Pipeline 的文档管理模块（`src/ingestion/document_manager.py`），负责跨存储的协调操作：
		...
		- `delete_document(source_path, collection) -> DeleteResult`：协调删除跨 4 个存储的关联数据：
			1. **Milvus** — 按 `metadata.source` 删除所有 chunk 向量
...
> **术语说明**：本节中的"提供者 (Provider)"、"实现 (Implementation)"指的是完成某项功能的**具体技术方案**，而非传统 Web 架构中的"后端服务器"。例如，LLM 提供者可以是远程的 Azure OpenAI API，也可以是本地运行的 Ollama；向量存储可以是本地嵌入式的 Milvus，也可以是云端托管的 Pinecone。本项目作为本地 MCP Server，通过统一接口对接这些不同的提供者，实现灵活切换。
...
本项目自定义了统一的 BaseVectorStore 抽象接口，暴露 .add()、.query()、.delete() 等方法。所有向量数据库后端（Milvus、Qdrant、Pinecone 等）只需实现该接口即可插拔替换，通过 VectorStoreFactory 根据配置自动选择具体实现。

本项目选用 **Milvus** 作为向量数据库。相比 Qdrant、Milvus、Weaviate 等需要 Docker 容器或分布式架构支撑的方案，Milvus 采用嵌入式设计，`pip install Milvusdb` 即可使用，无需额外部署数据库服务，非常适合本地开发与快速原型验证。同时 MilvusStore 适配器（src/libs/vector_store/Milvus_store.py），与 Pipeline 无缝集成。

> **当前实现说明**：目前系统仅实现了 Milvus 后端。虽然架构设计上预留了工厂模式以支持未来扩展，但当前版本尚未实现其他向量数据库的适配器。
...
| **Upsert** | 存储后端（method: Milvus）、upsert 数量、BM25 索引更新、图片存储、耗时 |
...
- **数据来源**：通过 `MilvusStore.get_all()` 或 `get_by_metadata()` 读取 chunk 数据。
...
└── services/
    ...
    ├── data_service.py       # 数据浏览服务（封装 MilvusStore/ImageStorage 读取）
...
- Dashboard 页面 1/2/3 直接读取存储层（通过 `DataService` 封装 MilvusStore/ImageStorage/FileIntegrity），不依赖 Trace。
...
|---------|---------|------|---------|------|
| A1 | 初始化目录树与最小可运行入口 | [x] | 2026-06-02 | |
| A2 | 引入 pytest 并建立测试目录约定 | [x] | 2026-06-02 | |
| A3 | 配置加载与校验（Settings） | [x] | 2026-06-02 | |

#### 阶段 B：Libs 可插拔层

| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| B1 | LLM 抽象接口与工厂 | [x] | 2026-06-02 | |
| B2 | Embedding 抽象接口与工厂 | [x] | 2026-06-02 | |
| B3 | Splitter 抽象接口与工厂 | [x] | 2026-06-02 | |
| B4 | VectorStore 抽象接口与工厂 | [x] | 2026-06-02 | |
| B5 | Reranker 抽象接口与工厂（含 None 回退） | [x] | 2026-06-02 | |
| B6 | Evaluator 抽象接口与工厂 | [x] | 2026-06-02 | |
...
| C2 | 文件完整性检查（SHA256） | [x] | 2026-06-09 | |
...
### B7.6：MilvusStore（VectorStore 默认后端）
- **目标**：补齐 `Milvus_store.py`，支持最小 `upsert(records)` 与 `query(vector, top_k, filters)`，并支持本地持久化目录（例如 `data/db/Milvus/`）。
- **修改文件**：
  - `src/libs/vector_store/Milvus_store.py`
  - `tests/integration/test_Milvus_store_roundtrip.py`
- **验收标准**：
  - provider=Milvus 时 `VectorStoreFactory` 可创建。
...
- **测试方法**：`pytest -q tests/integration/test_Milvus_store_roundtrip.py`
...
- **前置任务**：
  ...
  2. 需确认 MilvusStore.query() 返回结果包含 text（当前存储在 documents 字段，需补充返回）
- **修改文件**：
  - `src/core/types.py`（新增 `RetrievalResult` 类型）
  - `src/libs/vector_store/Milvus_store.py`（修复：query 返回结果需包含 text 字段）
...
- **验收标准**：
  ...
  - MilvusStore.query() 返回结果包含 `text` 字段
...
- **前置任务**：需在 `BaseVectorStore` 和 `MilvusStore` 中添加 `get_by_ids()` 方法，用于根据 chunk_id 批量获取 text 和 metadata
- **修改文件**：
  - `src/libs/vector_store/base_vector_store.py`（新增 `get_by_ids()` 抽象方法）
  - `src/libs/vector_store/Milvus_store.py`（实现 `get_by_ids()` 方法）
...
- **实现类/函数**：
  ...
  - `MilvusStore.get_by_ids(ids: List[str]) -> List[Dict]`：调用 MilvusDB 的 get 方法
...
- **验收标准**：
  - `BaseVectorStore.get_by_ids()` 和 `MilvusStore.get_by_ids()` 已实现
...
- **验收标准**：
  ...
  - 每个阶段记录 `elapsed_ms`、`method`（如 markitdown/recursive/Milvus）和处理详情
...
- **实现要点**：
  ...
  - Overview 页面：读取 `Settings` 展示组件卡片，调用 `MilvusStore.get_collection_stats()` 展示数据统计
...
- **修改文件**：
  ...
  - `src/libs/vector_store/Milvus_store.py`（增强：添加 `delete_by_metadata`）
...
- **实现类/函数**：
  - `DocumentManager.__init__(Milvus_store, bm25_indexer, image_storage, file_integrity)`
...
- **验收标准**：
  ...
  - `delete_document` 协调删除 Milvus + BM25 + ImageStorage + FileIntegrity 四个存储
  - 删除后再次 list 不包含已删除文档
...
- **修改文件**：
  ...
  - `src/observability/dashboard/services/data_service.py`（新增：封装 MilvusStore/ImageStorage 读取）
- **实现要点**：
  - 文档列表视图：展示 source_path、集合、chunk 数、摄入时间；支持集合筛选
  - Chunk 详情视图：点击文档展开所有 chunk，显示内容（可折叠）、metadata 字段、关联图片
  - `DataService`：封装 `MilvusStore.get_by_metadata()` 和 `ImageStorage.list_images()` 调用
